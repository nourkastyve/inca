<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Le debrouilalrd</title>
    <link rel="stylesheet" href="style/debrouillard.css">
</head>
<body>

    <nav>
        <h1>le debrouillard</h1><hr>
        <div class="nav">
            <a href="accueil.php">ACCUEIL</a><a href="vendre.php">VENTRE</a><a href="enregistrer.html">ENREGISTRER</a>
            <select>
                <option class="opa" value="" disabled selected>Diamant</option>
                <option value="option2"><a href="liste_commande.html">facture</a></option>
                <option value="option3"><a href="liste_diamant.html">Magasin</a></option>
              </select>
        </div><hr>
    </nav>

    <h1 style="text-align: center;">bienvenue chez le debrouillard</h1>
</body>
</html>